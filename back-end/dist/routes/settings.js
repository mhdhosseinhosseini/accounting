...
const createSchema = zod_1.z.object({
    ...
    special_id: zod_1.z.string().optional(),
    // Persisted field: controls UI behavior for this setting
    type: zod_1.z.enum(['special', 'digits', 'string'])
});
...
const updateSchema = zod_1.z.object({
    ...
    value: zod_1.z.any().optional(),
    special_id: zod_1.z.string().optional(),
    type: zod_1.z.enum(['special', 'digits', 'string']).optional()
});
/**
...
exports.settingsRouter.get('/', async (req, res) => {
    ...
    try {
        ...
        return res.json({ items: r.rows, message: (0, i18n_1.t)('settings.list', lang) });
    }
    catch (e) {
    ...
    try {
        ...
        if (isSpecial) {
            if (!isUuid(special_id)) {
                return res.status(400).json({ ok: false, error: (0, i18n_1.t)('settings.specialIdRequired', lang) });
            }
        }
        const valParam = isSpecial ? null : (value === undefined ? null : JSON.stringify(value));
        await p.query(`INSERT INTO settings (id, code, name, value, special_id, type, created_at, updated_at)
       VALUES ($1, $2, $3, $4::jsonb, $5, $6, NOW(), NOW())`, [id, code, name, valParam, special_id ?? null, type]);
        return res.status(201).json({ id, code, name, value: isSpecial ? null : value, special_id, type, message: (0, i18n_1.t)('settings.created', lang) });
    }
    catch (e) {
        if (String(e?.message || '').includes('unique') || String(e?.detail || '').includes('already exists')) {
            return res.status(409).json({ ok: false, error: (0, i18n_1.t)('settings.codeExists', lang) });
        }
 ...
 */
exports.settingsRouter.patch('/:id', async (req, res) => {
    const lang = req.lang || 'en';
    const { id } = req.params;
    ...
    try {
        ...
        if (data.special_id !== undefined) {
            params.push(data.special_id);
            sets.push(`special_id = $${params.length}`);
        }
        ...
        if (r.rowCount === 0)
            return res.status(404).json({ ok: false, error: (0, i18n_1.t)('settings.notFound', lang) });
        return res.json({ item: r.rows[0], message: (0, i18n_1.t)('settings.updated', lang) });
    }
    catch (e) {
        if (String(e?.message || '').includes('unique') || String(e?.detail || '').includes('already exists')) {
            return res.status(409).json({ ok: false, error: (0, i18n_1.t)('settings.codeExists', lang) });
 ...
 */
exports.settingsRouter.delete('/:id', async (req, res) => {
    const lang = req.lang || 'en';
    const { id } = req.params;
    try {
        const p = (0, pg_1.getPool)();
        ...
        if (r.rowCount === 0)
            return res.status(404).json({ ok: false, error: (0, i18n_1.t)('settings.notFound', lang) });
        return res.json({ ok: true, message: (0, i18n_1.t)('settings.deleted', lang) });
    }
    catch (e) {
