...
async function ensureSchema() {
  ...
  `);
    ...
    try {
        await p.query(`INSERT INTO settings (id, code, name, value, type, created_at, updated_at)
       SELECT $1, 'VITE_CASHBOX_START_CODE', 'کد شروع صندوق', '6000'::jsonb, 'digits', NOW(), NOW()
       WHERE NOT EXISTS (SELECT 1 FROM settings WHERE code = 'VITE_CASHBOX_START_CODE')`, [(0, crypto_1.randomUUID)()]);
        await p.query(`INSERT INTO settings (id, code, name, value, type, created_at, updated_at)
       SELECT $1, 'VITE_BANK_DETAIL_START_CODE', 'کد شروع جزئیات حساب بانکی', '6100'::jsonb, 'digits', NOW(), NOW()
       WHERE NOT EXISTS (SELECT 1 FROM settings WHERE code = 'VITE_BANK_DETAIL_START_CODE')`, [(0, crypto_1.randomUUID)()]);
    }
    catch { }
    /**
