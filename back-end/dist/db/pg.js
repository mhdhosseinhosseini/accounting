...
async function ensureSchema() {
  ...
  `);
    ...
    try {
        const existsSpecific = await p.query(`SELECT EXISTS (
    ...
    try {
        const existsProbe = await p.query(`SELECT EXISTS (
         SELECT 1 FROM information_schema.columns
    ...
    try {
        const detailIdProbe = await p.query(`SELECT EXISTS (
         SELECT 1 FROM information_schema.columns
  ...
  `);
    ...
    try {
        const rows = await p.query(`SELECT id, instrument_type, bank_account_id, card_reader_id, check_id
       FROM receipt_items
        ...
        for (const r of rows.rows || []) {
            ...
            if (inst === 'check' && r.check_id) {
                ...
                if (ex.rowCount && ex.rows[0]?.id) {
                    ...
                    linkId = (0, crypto_1.randomUUID)();
                    await p.query(`INSERT INTO instrument_links (id, instrument_type, check_id) VALUES ($1,'check',$2)`, [linkId, String(r.check_id)]);
                }
            }
                ...
                if (ex.rowCount && ex.rows[0]?.id) {
                    ...
                    linkId = (0, crypto_1.randomUUID)();
                    await p.query(`INSERT INTO instrument_links (id, instrument_type, card_reader_id) VALUES ($1,'card',$2)`, [linkId, String(r.card_reader_id)]);
                }
            }
                ...
                if (ex.rowCount && ex.rows[0]?.id) {
                    ...
                    linkId = (0, crypto_1.randomUUID)();
                    await p.query(`INSERT INTO instrument_links (id, instrument_type, bank_account_id) VALUES ($1,'transfer',$2)`, [linkId, String(r.bank_account_id)]);
                }
            }
            if (linkId) {
    ...
    // This block is idempotent and will skip if legacy item-level columns are absent.
    {
        ...
        const colCheck = await p.query(`SELECT column_name
       FROM information_schema.columns
       WHERE table_schema = 'public'
         AND table_name = 'receipt_items'
         AND column_name IN ('cashbox_id','destination_type','destination_id')`);
        const hasCashbox = colCheck.rows?.some((r) => r.column_name === 'cashbox_id');
        const hasDestType = colCheck.rows?.some((r) => r.column_name === 'destination_type');
        const hasDestId = colCheck.rows?.some((r) => r.column_name === 'destination_id');
        if (hasCashbox || (hasDestType && hasDestId)) {
            ...
            await p.query(`UPDATE receipts r SET cashbox_id = COALESCE(r.cashbox_id, sub.cashbox_id)
         FROM (
...
async function upsertUserByMobile(mobileNumber) {
    ...
    const res = await p.query(`INSERT INTO users (id, mobile_number) VALUES ($1, $2)
     ON CONFLICT (mobile_number) DO UPDATE SET mobile_number = EXCLUDED.mobile_number
     RETURNING id`, [id, mobileNumber]);
    return { id: res.rows[0].id };
}
/** Store a refresh token row with expiry. */
async function storeRefreshToken(token, userId, expiresAtMs) {
    const p = getPool();
    await p.query(`INSERT INTO refresh_tokens (token, user_id, revoked, expires_at)
     VALUES ($1, $2, FALSE, to_timestamp($3/1000.0))
     ON CONFLICT (token) DO NOTHING`, [token, userId, expiresAtMs]);
}
...
async function findRefreshToken(token) {
    const p = getPool();
    const res = await p.query(`SELECT rt.token, rt.revoked, rt.expires_at, u.mobile_number
