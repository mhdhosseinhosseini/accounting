...
function parseArgs() {
  ...
  const out: { src?: string; dest?: string; schema?: string; truncate?: boolean; dump?: string } = {};
  for (const a of args) {
    if (a.startsWith('--src=')) out.src = a.substring('--src='.length);
    else if (a.startsWith('--dest=')) out.dest = a.substring('--dest='.length);
    else if (a.startsWith('--schema=')) out.schema = a.substring('--schema='.length);
    else if (a === '--truncate') out.truncate = true;
    else if (a.startsWith('--dump=')) out.dump = a.substring('--dump='.length);
  }
  return out;
}
...
function checkBinaries(): boolean {
  ...
  let ok = true;
  for (const b of bins) {
    const res = spawnSync(b, ['--version'], { stdio: 'pipe' });
    if (res.status !== 0) {
 ...
 */
function parseConnOptions(url: string): { host: string; port: number; user?: string; db: string } {
  const u = new URL(url);
  return {
 ...
 */
function dumpSchemaData(srcUrl: string, schema: string, dumpFile: string): void {
  ...
  const res = spawnSync('pg_dump', ['--no-owner', '--no-privileges', `--schema=${schema}`, '--data-only', srcUrl, '-f', dumpFile], { stdio: 'inherit', env });
  if (res.status !== 0) {
 ...
 */
async function truncateTargetSchema(destUrl: string, schema: string): Promise<void> {
  console.log(`[INFO] Truncating destination schema '${schema}' tables (CASCADE) ...`);
  const pool = new Pool({ connectionString: destUrl });
  try {
    const q = `SELECT tablename FROM pg_tables WHERE schemaname = $1`;
    const r = await pool.query(q, [schema]);
    const tables = r.rows.map((row: any) => `${schema}."${row.tablename}"`);
    if (tables.length === 0) {
 ...
 */
function restoreSchemaData(destUrl: string, dumpFile: string, schemaForWrapper: string): void {
  ...
  const res = spawnSync('psql', ['-h', host, '-p', String(port), ...(user ? ['-U', user] : []), db, '-v', 'ON_ERROR_STOP=1', '-f', wrapper], { stdio: 'inherit', env });
  if (res.status !== 0) {
...
async function main() {
  ...
  if (!checkBinaries()) process.exit(2);
  ...
  if (truncate) await truncateTargetSchema(dest, schema);
 ...
 */
async function listSchemaTables(url: string, schema: string): Promise<string[]> {
 ...
 */
function dumpTableStructure(srcUrl: string, schema: string, table: string, ddlFile: string): void {
  ...
  const res = spawnSync(
    'pg_dump',
    ['--no-owner', '--no-privileges', `--schema=${schema}`, `--table=${schema}."${table}"`, '--schema-only', srcUrl, '-f', ddlFile],
    { stdio: 'inherit', env },
  );
  if (res.status !== 0) {
 ...
 */
function restoreDDLFile(destUrl: string, ddlFile: string): void {
  const { host, port, user, db } = parseConnOptions(destUrl);
  const u = new URL(destUrl);
  const env = { ...process.env, PGPASSWORD: u.password || '' };
  const res = spawnSync('psql', ['-h', host, '-p', String(port), ...(user ? ['-U', user] : []), db, '-v', 'ON_ERROR_STOP=1', '-f', ddlFile], {
    stdio: 'inherit',
    env,
  });
  if (res.status !== 0) {
 ...
 */
async function getForeignKeyEdges(url: string, schema: string): Promise<Array<{ child: string; parent: string }>> {
  const pool = new Pool({ connectionString: url });
  try {
    const q = `
    ...
    `;
    const r = await pool.query(q, [schema]);
    return r.rows.map((row: any) => ({ child: row.child, parent: row.parent }));
  } finally {
    await pool.end();
  }
}
 ...
 */
function topoOrderMissingTables(missing: string[], edges: Array<{ child: string; parent: string }>): string[] {
  ...
  if (order.length !== missing.length) {
    ...
    for (const t of missing) {
      if (!orderedSet.has(t)) order.push(t);
    }
  }
  return order;
}

async function ensureMissingTables(srcUrl: string, destUrl: string, schema: string): Promise<void> {
  ...
  for (let pass = 1; pass <= maxPasses; pass++) {
    ...
    for (const t of ordered) {
      ...
      try {
        ...
        console.warn(`[WARN] DDL restore failed for ${t}, will retry next pass: ${(e as Error).message}`);
      }
    }
 ...
 */
function createWrapperForDataImport(schema: string, dumpFile: string): string {
...
async function schemaHasTables(destUrl: string, schema: string): Promise<boolean> {
  const pool = new Pool({ connectionString: destUrl });
  try {
 ...
 */
async function getTableColumns(url: string, schema: string, table: string): Promise<Map<string, { type: string }>> {
  const pool = new Pool({ connectionString: url });
  try {
    const q = `
 ...
 */
function runPsqlInline(destUrl: string, sql: string): void {
  const { host, port, user, db } = parseConnOptions(destUrl);
  const u = new URL(destUrl);
  const env = { ...process.env, PGPASSWORD: u.password || '' };
  const res = spawnSync('psql', ['-h', host, '-p', String(port), ...(user ? ['-U', user] : []), db, '-v', 'ON_ERROR_STOP=1', '-c', sql], {
    stdio: 'inherit',
    env,
  });
  if (res.status !== 0) {
 ...
 */
async function ensureMissingColumns(srcUrl: string, destUrl: string, schema: string): Promise<void> {
  ...
  for (const t of common) {
    ...
    for (const [name, info] of srcCols.entries()) {
      if (!destCols.has(name)) missingCols.push({ name, type: info.type });
    }
 ...
 */
async function getColumnNullability(url: string, schema: string, table: string): Promise<Map<string, boolean>> {
  const pool = new Pool({ connectionString: url });
  try {
    const q = `
    ...
    for (const row of r.rows) m.set(row.name, row.notnull);
    return m;
  } finally {
    await pool.end();
  }
 ...
 */
async function ensureNullabilityCompatibility(srcUrl: string, destUrl: string, schema: string): Promise<void> {
  const [srcTables, destTables] = await Promise.all([
    listSchemaTables(srcUrl, schema),
    listSchemaTables(destUrl, schema),
  ]);
  ...
  for (const t of common) {
    ...
    const toRelax: string[] = [];
    for (const [name, srcNotNull] of srcNulls.entries()) {
      const destNotNull = destNulls.get(name);
      if (destNotNull === true && srcNotNull === false) toRelax.push(name);
    }
    if (toRelax.length === 0) continue;
 ...
 */
async function getColumnDefaults(url: string, schema: string, table: string): Promise<Map<string, string | null>> {
  const pool = new Pool({ connectionString: url });
  try {
    const q = `
    ...
    for (const row of r.rows) m.set(row.name, row.def ?? null);
    return m;
  } finally {
    await pool.end();
  }
 ...
 */
async function relaxDestExtraColumns(srcUrl: string, destUrl: string, schema: string): Promise<void> {
  const [srcTables, destTables] = await Promise.all([
    listSchemaTables(srcUrl, schema),
    listSchemaTables(destUrl, schema),
  ]);
  ...
  for (const t of common) {
    ...
    for (const name of destNames) {
      if (!srcNames.has(name)) extra.push(name);
    }
 ...
 */
async function listSchemaSequences(url: string, schema: string): Promise<string[]> {
  const pool = new Pool({ connectionString: url });
  try {
    const r = await pool.query(
    ...
    return r.rows.map((row: any) => row.sequence_name);
  } finally {
    await pool.end();
  }
}
 ...
 */
async function ensureMissingSequences(srcUrl: string, destUrl: string, schema: string): Promise<void> {
