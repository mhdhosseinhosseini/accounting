...
async function revertIncomingChecksRemovedFromReceipt(p: Pool, receiptId: string, prevCheckIds: string[], newCheckIds: string[]): Promise<void> {
  ...
  for (const cid of deletedIds) {
    // Find the instrument link for this check
    const li = await p.query(`SELECT id FROM instrument_links WHERE instrument_type='check' AND check_id=$1 LIMIT 1`, [cid]);
    const linkId = li.rowCount ? String(li.rows[0].id) : null;
    if (!linkId) continue;

    // If no receipt items reference this check anymore, revert status
    const ref = await p.query(`SELECT COUNT(1) AS cnt FROM receipt_items WHERE related_instrument_id=$1`, [linkId]);
    const cnt = Number(ref.rows?.[0]?.cnt || 0);
    if (cnt === 0) {
      await p.query(
        `UPDATE checks SET status='created'
           WHERE id=$1
             AND type='incoming'
             AND checkbook_id IS NULL
...
treasuryRouter.post('/receipts', async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const it of items) {
      ...
      let relatedInstrumentId: string | null = null;
      if (inst === 'card') {
        relatedInstrumentId = await getOrCreateInstrumentLink(p as any, 'card', cardReaderId);
      } else if (inst === 'transfer') {
        relatedInstrumentId = await getOrCreateInstrumentLink(p as any, 'transfer', bankAccountId);
      } else if (inst === 'check') {
        relatedInstrumentId = await getOrCreateInstrumentLink(p as any, 'check', checkId);
      }

      await p.query(
        `INSERT INTO receipt_items (id, receipt_id, instrument_type, amount, reference, related_instrument_id, position)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [iid, id, inst, amount, reference, relatedInstrumentId, position]
      );
    }
    ...
    const ir = await p.query(
      `SELECT ri.id, ri.instrument_type, ri.amount, il.bank_account_id, il.card_reader_id, ri.reference, il.check_id, ri.position
       FROM receipt_items ri
       LEFT JOIN instrument_links il ON il.id = ri.related_instrument_id
       WHERE ri.receipt_id = $1
...
treasuryRouter.get('/payments', async (req: Request, res: Response) => {
  ...
  try {
    const p = getPool();
    const { rows } = await p.query(
      `SELECT id, number, status, date, detail_id, total_amount, fiscal_year_id, created_at
       FROM payments
       ORDER BY date DESC, created_at DESC
       LIMIT 200`
    );
    return res.json({ ok: true, items: rows, message: t('treasury.payments.list', lang) });
  } catch (e: any) {
    return res.status(500).json({ ok: false, error: e?.message || 'Error' });
  }
});
